...
export default function ShippedOrder() {
  ...
  const v = {
    ...
    tracking_number: val(variables.tracking_number, 'TRACKING_NUMBER'),
    carrier: variables.carrier || '',
    tracking_url: variables.tracking_url || '',
    estimated_delivery: val(
      formatDate(variables.estimated_delivery),
      'EST_DELIVERY',
    ),
  ...
  async function onCopy() {
    try {
      const res = await fetch('/api/orders/shipped/html', {
        method: 'POST',
  ...
  async function onSend(data: Variables) {
    try {
      const res = await fetch('/api/orders/shipped/send', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
          ...
          <ScrollArea className="flex-1">
            ...
            >
              <Flex direction="column" gap="3">
                ...
                >
                  <DateInput
                    value={variables.estimated_delivery}
            ...
            <div className="px-12 py-10">
              ...
              {design.showTracking && (
                ...
                >
                  ...
                  {v.carrier && (
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-zinc-500 text-[13px]">Carrier</span>
                      <span className="font-medium text-zinc-700 text-[13px]">
                        {v.carrier}
                      </span>
                    </div>
                  )}
                  {variables.estimated_delivery && (
                    <div className="flex items-center justify-between">
                      <span className="text-zinc-500 text-[13px]">
                        Est. delivery
                      </span>
                      <span className="font-medium text-zinc-700 text-[13px]">
                        {ph(v.estimated_delivery)}
                      </span>
                    </div>
                  )}
                  {v.tracking_url && (
