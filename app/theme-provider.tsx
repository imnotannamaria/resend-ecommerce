...
export default function ThemeProvider({
  ...
  return (
    <Theme appearance={appearance}>
      <header
        className={`px-5 py-0 border-b flex items-center justify-between h-14 ${isDark ? 'border-zinc-800' : 'border-zinc-100'}`}
      >
        <Link href="/" className="flex items-center gap-2.5 shrink-0">
          <div
            className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 transition-colors ${isDark ? 'bg-white' : 'bg-zinc-900'}`}
          >
            <AtSign
              size={14}
            ...
            return (
              <div key={href} className="flex items-center gap-0.5 sm:gap-1">
                {index > 0 && (
                  <ChevronRight
                    size={12}
                    className={`hidden sm:block ${isDark ? 'text-zinc-600' : 'text-zinc-300'}`}
                  />
                )}
                <Link
                  href={href}
                  className={`flex items-center gap-1.5 px-2 py-1.5 rounded-md text-sm font-medium transition-all duration-150 sm:px-3
                    ${
                      active
                        ? isDark
